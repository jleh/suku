...
export default (persons, rootPersonId) => {
  ...
  const findParents = (person, personMap, processedPersons, generation) => {
    ...
    if (exsistingPerson) {
      ...
      parents = [
        person.father ? findParents(personMap.get(person.father), personMap, processedPersons, generation + 1) : {},
        person.mother ? findParents(personMap.get(person.mother), personMap, processedPersons, generation + 1) : {}
      ];
    }
